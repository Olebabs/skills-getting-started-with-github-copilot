        ...
        const participantsHtml = details.participants.length
          ? `<p><strong>Participants:</strong></p>
             <ul class="participants-list">
               ${details.participants.map(p => `<li>${p} <button class="remove-participant" data-email="${p}" aria-label="Remove ${p}">✖</button></li>`).join("\n")}
             </ul>`
          ...
          btn.addEventListener("click", async () => {
            ...
            try {
              ...
              if (res.ok) {
                ...
                fetchActivities(); // refresh list to reflect removal
              } else {
